import { _ as resolveStateDir, g as resolveOAuthPath, i as init_paths$1, v as __esmMin, y as __exportAll } from "./paths-7Bb9F26w.js";
import { c as init_session_key, n as DEFAULT_AGENT_ID } from "./session-key-CD-TlcsJ.js";
import { n as init_subsystem, t as createSubsystemLogger } from "./subsystem-D_qnfThk.js";
import { g as resolveUserPath, l as init_utils } from "./utils-D-0UeDZ-.js";
import { o as resolveAgentModelPrimary, t as init_agent_scope } from "./agent-scope-BKxriAmP.js";
import { a as init_json_file, i as resolveCopilotApiToken, o as loadJsonFile, r as init_github_copilot_token, s as saveJsonFile, t as DEFAULT_COPILOT_API_BASE_URL } from "./github-copilot-token-DeDg-5yZ.js";
import { n as init_command_format, t as formatCliCommand } from "./command-format-D7BB3K65.js";
import { n as isTruthyEnvValue, t as init_env } from "./env-B64iiAJQ.js";
...
var DEFAULT_PROVIDER, DEFAULT_MODEL, DEFAULT_CONTEXT_TOKENS;
...
var AUTH_STORE_VERSION, AUTH_PROFILE_FILENAME, LEGACY_AUTH_FILENAME, CLAUDE_CLI_PROFILE_ID, CODEX_CLI_PROFILE_ID, QWEN_CLI_PROFILE_ID, MINIMAX_CLI_PROFILE_ID, AUTH_STORE_LOCK_OPTIONS, EXTERNAL_CLI_SYNC_TTL_MS, EXTERNAL_CLI_NEAR_EXPIRY_MS, log$1;
var init_constants = __esmMin((() => {
	...
	EXTERNAL_CLI_SYNC_TTL_MS = 900 * 1e3;
	EXTERNAL_CLI_NEAR_EXPIRY_MS = 600 * 1e3;
	log$1 = createSubsystemLogger("agents/auth-profiles");
}));
...
var log, QWEN_CLI_CREDENTIALS_RELATIVE_PATH, MINIMAX_CLI_CREDENTIALS_RELATIVE_PATH, qwenCliCache, minimaxCliCache;
var init_cli_credentials = __esmMin((() => {
	init_json_file();
	init_subsystem();
	init_utils();
	log = createSubsystemLogger("agents/auth-profiles");
	QWEN_CLI_CREDENTIALS_RELATIVE_PATH = ".qwen/oauth_creds.json";
	MINIMAX_CLI_CREDENTIALS_RELATIVE_PATH = ".minimax/oauth_creds.json";
	qwenCliCache = null;
...
var init_external_cli_sync = __esmMin((() => {
	init_cli_credentials();
	init_constants();
}));
...
var QWEN_OAUTH_TOKEN_ENDPOINT, QWEN_OAUTH_CLIENT_ID;
var init_qwen_portal_oauth = __esmMin((() => {
	init_command_format();
...
var CHUTES_OAUTH_ISSUER, CHUTES_AUTHORIZE_ENDPOINT, CHUTES_TOKEN_ENDPOINT, CHUTES_USERINFO_ENDPOINT, DEFAULT_EXPIRES_BUFFER_MS;
var init_chutes_oauth = __esmMin((() => {
	CHUTES_OAUTH_ISSUER = "https://api.chutes.ai";
	CHUTES_AUTHORIZE_ENDPOINT = `${CHUTES_OAUTH_ISSUER}/idp/authorize`;
	CHUTES_TOKEN_ENDPOINT = `${CHUTES_OAUTH_ISSUER}/idp/token`;
	CHUTES_USERINFO_ENDPOINT = `${CHUTES_OAUTH_ISSUER}/idp/userinfo`;
	DEFAULT_EXPIRES_BUFFER_MS = 300 * 1e3;
}));
...
var OAUTH_PROVIDER_IDS, isOAuthProvider, resolveOAuthProvider;
var init_oauth = __esmMin((() => {
	init_qwen_portal_oauth();
	init_chutes_oauth();
	init_constants();
	init_doctor();
	init_paths();
	init_repair();
	init_store();
	OAUTH_PROVIDER_IDS = new Set(getOAuthProviders().map((provider) => provider.id));
	isOAuthProvider = (provider) => OAUTH_PROVIDER_IDS.has(provider);
	resolveOAuthProvider = (provider) => isOAuthProvider(provider) ? provider : null;
}));
...
var init_auth_profiles = __esmMin((() => {
	init_constants();
	init_display();
	init_doctor();
	init_oauth();
...
var DEFAULT_REFRESH_INTERVAL_SECONDS, DEFAULT_CONTEXT_WINDOW, DEFAULT_MAX_TOKENS, DEFAULT_COST, discoveryCache, hasLoggedBedrockError;
var init_bedrock_discovery = __esmMin((() => {
	DEFAULT_REFRESH_INTERVAL_SECONDS = 3600;
	DEFAULT_CONTEXT_WINDOW = 32e3;
...
var CLOUDFLARE_AI_GATEWAY_PROVIDER_ID, CLOUDFLARE_AI_GATEWAY_DEFAULT_MODEL_ID, CLOUDFLARE_AI_GATEWAY_DEFAULT_MODEL_REF, CLOUDFLARE_AI_GATEWAY_DEFAULT_CONTEXT_WINDOW, CLOUDFLARE_AI_GATEWAY_DEFAULT_MAX_TOKENS, CLOUDFLARE_AI_GATEWAY_DEFAULT_COST;
var init_cloudflare_ai_gateway = __esmMin((() => {
	CLOUDFLARE_AI_GATEWAY_PROVIDER_ID = "cloudflare-ai-gateway";
	CLOUDFLARE_AI_GATEWAY_DEFAULT_MODEL_ID = "claude-sonnet-4-5";
	CLOUDFLARE_AI_GATEWAY_DEFAULT_MODEL_REF = `${CLOUDFLARE_AI_GATEWAY_PROVIDER_ID}/${CLOUDFLARE_AI_GATEWAY_DEFAULT_MODEL_ID}`;
	CLOUDFLARE_AI_GATEWAY_DEFAULT_CONTEXT_WINDOW = 2e5;
	CLOUDFLARE_AI_GATEWAY_DEFAULT_MAX_TOKENS = 64e3;
...
var DEFAULT_TIMEOUT_MS, DEFAULT_MAX_BUFFER_BYTES, lastAppliedKeys, cachedShellPath;
var init_shell_env = __esmMin((() => {
	init_env();
...
var AWS_BEARER_ENV, AWS_ACCESS_KEY_ENV, AWS_SECRET_KEY_ENV, AWS_PROFILE_ENV;
var init_model_auth = __esmMin((() => {
	init_command_format();
	init_shell_env();
	init_auth_profiles();
	init_model_selection();
	AWS_BEARER_ENV = "AWS_BEARER_TOKEN_BEDROCK";
...
var SYNTHETIC_BASE_URL, SYNTHETIC_DEFAULT_MODEL_ID, SYNTHETIC_DEFAULT_MODEL_REF, SYNTHETIC_DEFAULT_COST, SYNTHETIC_MODEL_CATALOG;
var init_synthetic_models = __esmMin((() => {
	SYNTHETIC_BASE_URL = "https://api.synthetic.new/anthropic";
	SYNTHETIC_DEFAULT_MODEL_ID = "hf:MiniMaxAI/MiniMax-M2.1";
	SYNTHETIC_DEFAULT_MODEL_REF = `synthetic/${SYNTHETIC_DEFAULT_MODEL_ID}`;
...
var VENICE_BASE_URL, VENICE_DEFAULT_MODEL_ID, VENICE_DEFAULT_MODEL_REF, VENICE_DEFAULT_COST, VENICE_MODEL_CATALOG;
var init_venice_models = __esmMin((() => {
	VENICE_BASE_URL = "https://api.venice.ai/api/v1";
	VENICE_DEFAULT_MODEL_ID = "llama-3.3-70b";
	VENICE_DEFAULT_MODEL_REF = `venice/${VENICE_DEFAULT_MODEL_ID}`;
	VENICE_DEFAULT_COST = {
		input: 0,
		output: 0,
		cacheRead: 0,
...
async function resolveImplicitProviders(params) {
	...
	const siliconKey = resolveEnvApiKeyVarName("siliconflow") ?? resolveApiKeyFromProfiles({
		provider: "siliconflow",
		store: authStore
	});
	if (siliconKey) providers.siliconflow = {
		...buildSiliconflowProvider(),
		apiKey: siliconKey
	};
	const dashscopeKey = resolveEnvApiKeyVarName("dashscope") ?? resolveApiKeyFromProfiles({
		provider: "dashscope",
		store: authStore
	});
	if (dashscopeKey) providers.dashscope = {
		...buildDashscopeProvider(),
		apiKey: dashscopeKey
	};
	const deepseekKey = resolveEnvApiKeyVarName("dpseek") ?? resolveApiKeyFromProfiles({
		provider: "deepseek",
		store: authStore
	});
	...
	if (volcengineKey) providers.volcengine = {
		...buildVolcengineProvider(),
		apiKey: volcengineKey
	};
	const kimiCodeKey = resolveEnvApiKeyVarName("kimi-code") ?? resolveApiKeyFromProfiles({
		provider: "kimi-code",
		store: authStore
	});
	if (kimiCodeKey) providers["kimi-code"] = {
...
var MINIMAX_API_BASE_URL, MINIMAX_PORTAL_BASE_URL, MINIMAX_DEFAULT_MODEL_ID, MINIMAX_DEFAULT_VISION_MODEL_ID, MINIMAX_DEFAULT_CONTEXT_WINDOW, MINIMAX_DEFAULT_MAX_TOKENS, MINIMAX_OAUTH_PLACEHOLDER, MINIMAX_API_COST, XIAOMI_DEFAULT_MODEL_ID, MOONSHOT_BASE_URL, MOONSHOT_DEFAULT_MODEL_ID, MOONSHOT_DEFAULT_CONTEXT_WINDOW, MOONSHOT_DEFAULT_MAX_TOKENS, MOONSHOT_DEFAULT_COST, QWEN_PORTAL_BASE_URL, QWEN_PORTAL_OAUTH_PLACEHOLDER, QWEN_PORTAL_DEFAULT_CONTEXT_WINDOW, QWEN_PORTAL_DEFAULT_MAX_TOKENS, QWEN_PORTAL_DEFAULT_COST, OLLAMA_BASE_URL, OLLAMA_API_BASE_URL, OLLAMA_DEFAULT_CONTEXT_WINDOW, OLLAMA_DEFAULT_MAX_TOKENS, OLLAMA_DEFAULT_COST, QIANFAN_BASE_URL, QIANFAN_DEFAULT_MODEL_ID, QIANFAN_DEFAULT_CONTEXT_WINDOW, QIANFAN_DEFAULT_MAX_TOKENS, QIANFAN_DEFAULT_COST, VOLCENGINE_API_BASE_URL, XIAOMI_API_BASE_URL, XIAOMI_COMPAT;
var init_models_config_providers = __esmMin((() => {
	init_github_copilot_token();
	init_auth_profiles();
	init_bedrock_discovery();
	init_cloudflare_ai_gateway();
	init_model_auth();
	init_synthetic_models();
	init_venice_models();
	MINIMAX_API_BASE_URL = "https://api.minimax.chat/v1";
	MINIMAX_PORTAL_BASE_URL = "https://api.minimax.io/anthropic";
	MINIMAX_DEFAULT_MODEL_ID = "MiniMax-M2.1";
	MINIMAX_DEFAULT_VISION_MODEL_ID = "MiniMax-VL-01";
	MINIMAX_DEFAULT_CONTEXT_WINDOW = 2e5;
	MINIMAX_DEFAULT_MAX_TOKENS = 8192;
	MINIMAX_OAUTH_PLACEHOLDER = "minimax-oauth";
	MINIMAX_API_COST = {
		input: 15,
		output: 60,
		cacheRead: 2,
	...
	QWEN_PORTAL_OAUTH_PLACEHOLDER = "qwen-oauth";
	QWEN_PORTAL_DEFAULT_CONTEXT_WINDOW = 128e3;
	QWEN_PORTAL_DEFAULT_MAX_TOKENS = 8192;
	QWEN_PORTAL_DEFAULT_COST = {
		input: 0,
		output: 0,
		cacheRead: 0,
		cacheWrite: 0
	...
	OLLAMA_DEFAULT_CONTEXT_WINDOW = 128e3;
	...
	QIANFAN_DEFAULT_MODEL_ID = "deepseek-v3.2";
	QIANFAN_DEFAULT_CONTEXT_WINDOW = 98304;
	QIANFAN_DEFAULT_MAX_TOKENS = 32768;
	QIANFAN_DEFAULT_COST = {
		input: 0,
		output: 0,
		cacheRead: 0,
		cacheWrite: 0
	...
	XIAOMI_COMPAT = { supportsDeveloperRole: false };
}));
...
var init_model_selection = __esmMin((() => {
	init_agent_scope();
	init_defaults();
...
//#endregion
export { shouldDeferShellEnvFallback as $, VENICE_BASE_URL as A, init_agent_paths as At, getApiKeyForModel as B, buildXiaomiProvider as C, ensureAuthProfileStore as Ct, resolveImplicitBedrockProvider as D, init_paths as Dt, normalizeProviders as E, updateAuthProfileStoreWithLock as Et, SYNTHETIC_BASE_URL as F, DEFAULT_CONTEXT_TOKENS as Ft, resolveAwsSdkEnvVarName as G, init_model_auth as H, SYNTHETIC_DEFAULT_MODEL_REF as I, DEFAULT_MODEL as It, getShellEnvAppliedKeys as J, resolveEnvApiKey as K, SYNTHETIC_MODEL_CATALOG as L, DEFAULT_PROVIDER as Lt, VENICE_MODEL_CATALOG as M, resolveAuthProfileDisplayLabel as Mt, buildVeniceModelDefinition as N, CLAUDE_CLI_PROFILE_ID as Nt, resolveImplicitCopilotProvider as O, resolveAuthStorePath as Ot, init_venice_models as P, CODEX_CLI_PROFILE_ID as Pt, resolveShellEnvFallbackTimeoutMs as Q, buildSyntheticModelDefinition as R, init_defaults as Rt, buildQianfanProvider as S, upsertAuthProfile as St, normalizeGoogleModelId as T, loadAuthProfileStore as Tt, requireApiKey as U, getCustomProviderApiKey as V, resolveApiKeyForProvider as W, init_shell_env as X, getShellPathFromLoginShell as Y, loadShellEnvFallback as Z, QIANFAN_BASE_URL as _, parseOAuthCallbackInput as _t, init_model_selection as a, auth_profiles_exports as at, XIAOMI_API_BASE_URL as b, markAuthProfileGood as bt, normalizeProviderId as c, isProfileInCooldown as ct, resolveAllowlistModelKey as d, resolveProfileUnusableUntilForDisplay as dt, shouldEnableShellEnvFallback as et, resolveConfiguredModelRef as f, resolveApiKeyForProfile as ft, resolveThinkingDefault as g, init_chutes_oauth as gt, resolveModelRefFromString as h, generateChutesPkce as ht, getModelRefStatus as i, resolveCloudflareAiGatewayBaseUrl as it, VENICE_DEFAULT_MODEL_REF as j, resolveOpenClawAgentDir as jt, resolveImplicitProviders as k, resolveAuthStorePathForDisplay as kt, parseModelRef as l, markAuthProfileFailure as lt, resolveHooksGmailModel as m, exchangeChutesCodeForTokens as mt, buildConfiguredAllowlistKeys as n, buildCloudflareAiGatewayModelDefinition as nt, isCliProvider as o, init_auth_profiles as ot, resolveDefaultModelForAgent as p, CHUTES_AUTHORIZE_ENDPOINT as pt, resolveModelAuthMode as q, buildModelAliasIndex as r, init_cloudflare_ai_gateway as rt, modelKey as s, resolveAuthProfileOrder as st, buildAllowedModelSet as t, CLOUDFLARE_AI_GATEWAY_DEFAULT_MODEL_REF as tt, resolveAllowedModelRef as u, markAuthProfileUsed as ut, QIANFAN_DEFAULT_MODEL_ID as v, repairOAuthProfileIdMismatch as vt, init_models_config_providers as w, init_store as wt, XIAOMI_DEFAULT_MODEL_ID as x, setAuthProfileOrder as xt, VOLCENGINE_API_BASE_URL as y, listProfilesForProvider as yt, init_synthetic_models as z };