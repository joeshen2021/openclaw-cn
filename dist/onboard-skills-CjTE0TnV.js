import { Dr as loadModelCatalog, Dt as openUrl, Mt as resolveNodeManagerOptions, yt as detectBinary } from "./reply-Cq9qFKEL.js";
import { g as resolveUserPath, l as init_utils, p as resolveConfigDir, s as ensureDir, t as CONFIG_DIR } from "./utils-D-0UeDZ-.js";
import { n as runCommandWithTimeout, t as init_exec } from "./exec-nNwfnVsi.js";
import { E as resolveDefaultAgentWorkspaceDir, c as resolveAgentWorkspaceDir, i as resolveAgentDir, l as resolveDefaultAgentId, o as resolveAgentModelPrimary, t as init_agent_scope, w as init_workspace } from "./agent-scope-BKxriAmP.js";
import { At as init_agent_paths, Ct as ensureAuthProfileStore, H as init_model_auth, I as SYNTHETIC_DEFAULT_MODEL_REF, It as DEFAULT_MODEL, K as resolveEnvApiKey, Lt as DEFAULT_PROVIDER, Rt as init_defaults, St as upsertAuthProfile, V as getCustomProviderApiKey, _t as parseOAuthCallbackInput, a as init_model_selection, b as XIAOMI_API_BASE_URL, c as normalizeProviderId, d as resolveAllowlistModelKey, f as resolveConfiguredModelRef, gt as init_chutes_oauth, ht as generateChutesPkce, j as VENICE_DEFAULT_MODEL_REF, jt as resolveOpenClawAgentDir, mt as exchangeChutesCodeForTokens, ot as init_auth_profiles, pt as CHUTES_AUTHORIZE_ENDPOINT, r as buildModelAliasIndex, s as modelKey, st as resolveAuthProfileOrder, t as buildAllowedModelSet, tt as CLOUDFLARE_AI_GATEWAY_DEFAULT_MODEL_REF, w as init_models_config_providers, y as VOLCENGINE_API_BASE_URL, yt as listProfilesForProvider } from "./model-selection-B4yu6Rbo.js";
import { n as init_command_format, t as formatCliCommand } from "./command-format-D7BB3K65.js";
import { _ as resolveSkillKey, d as hasBinary, i as loadWorkspaceSkillEntries, t as resolveSkillsInstallPreferences } from "./skills-Vf9A_Paw.js";
import { Y as fetchWithSsrFGuard } from "./deliver-DTGGDD9R.js";
import { t as resolveBrewExecutable } from "./brew-Bc4CV7eU.js";
import { $ as setCloudflareAiGatewayConfig, A as applyMoonshotProviderConfigCn, B as applyVeniceProviderConfig, C as applyDeepseekConfig, Ct as SILICONFLOW_DEFAULT_MODEL_REF, D as applyMoonshotConfig, E as applyKimiCodeProviderConfig, Et as validateAnthropicSetupToken, F as applySiliconflowConfig, G as applyXiaomiConfig, H as applyVercelAiGatewayProviderConfig, I as applySiliconflowProviderConfig, J as OPENROUTER_DEFAULT_MODEL_REF, K as applyXiaomiProviderConfig, L as applySyntheticConfig, M as applyOpenrouterProviderConfig, N as applyQianfanConfig, O as applyMoonshotConfigCn, P as applyQianfanProviderConfig, Q as setAnthropicApiKey, R as applySyntheticProviderConfig, S as applyDashscopeProviderConfig, St as QIANFAN_DEFAULT_MODEL_REF, T as applyKimiCodeConfig, Tt as buildTokenProfileId, U as applyXaiConfig, V as applyVercelAiGatewayConfig, W as applyXaiProviderConfig, X as XIAOMI_DEFAULT_MODEL_REF, Y as VERCEL_AI_GATEWAY_DEFAULT_MODEL_REF, Z as ZAI_DEFAULT_MODEL_REF, _ as applyMinimaxProviderConfig, _t as writeOAuthCredentials, at as setMoonshotApiKey, b as applyCloudflareAiGatewayProviderConfig, bt as KIMI_CODING_MODEL_REF, ct as setQianfanApiKey, d as resolvePluginProviders, dt as setVeniceApiKey, et as setDashscopeApiKey, f as applyOpencodeZenConfig, ft as setVercelAiGatewayApiKey, g as applyMinimaxConfig, gt as setZaiApiKey, h as applyMinimaxApiProviderConfig, ht as setXiaomiApiKey, i as formatTokenK, it as setMinimaxApiKey, j as applyOpenrouterConfig, k as applyMoonshotProviderConfig, l as createVpsAwareOAuthHandlers, lt as setSiliconflowApiKey, m as applyMinimaxApiConfig, mt as setXaiApiKey, nt as setGeminiApiKey, ot as setOpencodeZenApiKey, p as applyOpencodeZenProviderConfig, pt as setVolcengineApiKey, q as applyZaiConfig, rt as setKimiCodingApiKey, st as setOpenrouterApiKey, t as githubCopilotLoginCommand, tt as setDeepseekApiKey, u as isRemoteEnvironment, ut as setSyntheticApiKey, v as applyAuthProfileConfig, vt as DASHSCOPE_DEFAULT_MODEL_REF, w as applyDeepseekProviderConfig, wt as XAI_DEFAULT_MODEL_REF, x as applyDashscopeConfig, xt as MOONSHOT_DEFAULT_MODEL_REF, y as applyCloudflareAiGatewayConfig, yt as DEEPSEEK_DEFAULT_MODEL_REF, z as applyVeniceConfig } from "./github-copilot-auth-E8lYNwd4.js";
import { a as enablePluginInConfig } from "./onboard-channels-oAasJI9j.js";
import { t as scanDirectoryWithSummary } from "./skill-scanner-CXg3PnU6.js";
import { t as buildWorkspaceSkillStatus } from "./skills-status-pFTOIQ8d.js";
import { n as resolveWideAreaDiscoveryDomain } from "./widearea-dns-C1pZ3gGH.js";
...
function buildAuthChoiceOptions(params) {
	...
	options.push({
		...
		hint: "硅基流动 - OpenAI compatible"
	});
	options.push({
		value: "dashscope-api-key",
		label: "DashScope API key",
		hint: "阿里云百炼 - OpenAI compatible"
	});
	options.push({
		value: "deepseek-api-key",
		label: "DeepSeek API key",
		hint: "DeepSeek - OpenAI compatible"
	});
	options.push({
		value: "volcengine-api-key",
		label: "Volcengine API key",
		hint: "火山引擎 ARK - OpenAI compatible"
	});
	options.push({
...
async function applyAuthChoiceApiProviders(params) {
	...
	if (authChoice === "dashscope-api-key") {
		...
		if (envKey && !hasCredential) {
			if (await params.prompter.confirm({
				message: `Use existing DASHSCOPE_API_KEY (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
				initialValue: true
			})) {
...
async function applyAuthChoiceVolcengine(params) {
	...
	if (params.opts?.tokenProvider === "volcengine" && params.opts?.token) apiKey = params.opts.token;
	if (params.opts?.volcengineApiKey) apiKey = params.opts.volcengineApiKey;
	if (apiKey) {
		if (!await params.prompter.confirm({
	...
	const verifyModelAccess = async (id) => {
		...
		try {
			...
			if (process.env.MODEL_AGENT_CLIENT_REQ_ID && process.env.MODEL_AGENT_CLIENT_REQ_VALUE) headers[process.env.MODEL_AGENT_CLIENT_REQ_ID] = process.env.MODEL_AGENT_CLIENT_REQ_VALUE;
			...
			if (!res.ok) {
				const errMsg = (await res.json().catch(() => ({})))?.error?.message || res.statusText;
			...
			await params.prompter.note(`Model "${id}" verification failed:\n${err.message}\n\nTip: You may need to create an Endpoint in ARK console or enable Pay-as-you-go.`, "Validation Error");
			return false;
		}
	};
	while (!modelId) {
		...
		if (selection === "__manual__") {
			const input = await params.prompter.text({
				...
				validate: (val) => val.length > 0 ? void 0 : "Endpoint ID is required"
			});
			if (typeof input === "symbol") return null;
			candidateId = String(input);
		} else candidateId = String(selection);
		if (await verifyModelAccess(candidateId)) modelId = candidateId;
		else selectionMessage = "Access Denied - Please ensure you have activated this model/endpoint in ARK Console";
	}
...
//#endregion
//#region src/commands/auth-choice.apply.xiaomi.ts
init_model_auth();
init_models_config_providers();
async function applyAuthChoiceXiaomi(params) {
	if (params.authChoice !== "xiaomi-api-key") return null;
	let apiKey = resolveEnvApiKey("xiaomi")?.apiKey;
	if (params.opts?.tokenProvider === "xiaomi" && params.opts?.token) apiKey = params.opts.token;
	if (params.opts?.xiaomiApiKey) apiKey = params.opts.xiaomiApiKey;
	if (apiKey) {
		if (!await params.prompter.confirm({
	...
	const verifyModelAccess = async (id) => {
		...
		try {
			...
			await params.prompter.note(`模型 "${id}" 验证失败:\n${err.message}\n\n提示: 请确保您已在 platform.xiaomimimo.com 开通服务并充值。`, "验证错误");
			return false;
		}
	};
	while (!modelId) {
		const choices = [...["mimo-v2-flash"].map((id) => ({
			value: id,
			label: id,
		...
		if (selection === "__manual__") {
			const input = await params.prompter.text({
				...
				validate: (val) => val.length > 0 ? void 0 : "模型ID不能为空"
			});
			if (typeof input === "symbol") return null;
			candidateId = String(input);
		} else candidateId = String(selection);
		if (await verifyModelAccess(candidateId)) modelId = candidateId;
		else selectionMessage = "访问被拒绝 - 请确保您已开通该模型服务";
	}
	let nextConfig = applyAuthProfileConfig(params.config, {
		profileId: "xiaomi:default",
		provider: "xiaomi",
